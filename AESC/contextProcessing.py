...
#inputpath:原始训练数据集     deppath：相应的依存关系    outpath：输出为cox类型的文件存储上下文
def createFeatureFileForPol(inputpath,deppath,outpath,context,d_type):  
    ...
    aspectTermList=[]      #存放Aspect
    texts_list=[]          #存放语句
    aspectContextList=[]   #存放Aspect的上下文关联文本
...
def createAllForPol(d_type='re',context=5):   #上下文窗口采用5
    if d_type=='re':
