...
def loadXML(filename):
    ...
    corpus=entity.Corpus(elements)     #调用语料（
    return corpus  
  
#获取BIO的全部信息      将数据中的{from:,to:}转化为BIO标注形式
def createBIOClass(instances,d_type):
    bio_entity=entity.BIO_Entity(instances,d_type)
    bio_entity.createBIOTags()    #BIO标注
    bio_entity.createPOSTags()    #词性
    bio_entity.createLemm()       #词干
    
    bio_entity.createW2VCluster()  #词向量
    ...
    if d_type=='re':
        ...
        with open(filepath,'r') as f:
            ...
            for item in items_list:
                ...
                if 'notebook' in words1 or 'laptop' in words1:  #选取amzon文件中的笔记本有关的文件
...
#获取餐厅数据的句法依存关系,一共有3041个句子，3693个AspectTerm,1288个不同的AspectTerm
#SemEval-2014challenge,restaurants一共有800个句子，1134个AspectTerm,555个不同的AspectTerm

#获取笔记本数据的句法依存关系,一共有3045个句子，2358个AspectTerm,1042个不同的AspectTerm
#SemEval-2014challenge,lapTops一共有800个句子，654个AspectTerm,419个不同的AspectTerm
...
def createDependenceInformation(inputfile,outputfile):
    ...
    for text in texts:
        ...
        # print(sents)
        # print(len(sents))
        all_sents.extend(sents)     #对句子分词后的结果  ['Read', 'all', 'the', 'other', 'review', ',', 'they', "'ll", 'confirm', 'it', '.']
        sent_num.append(len(sents))  #统计句子的数量  [1,....]
    ...
    res_list=list(eng_parser.parse_sents(all_sents))   #依存法分析
    ...
    print('切分完成，开始保存')
    #保存在dependences/re_test.dep和  dependences/lp_test.dep文件中
...
if __name__=='__main__': 
    ...
    #将训练集和测试集分别保存在依存关系中 re_train.dep，re_test.dep,lp_train.dep,lp_test.dep
    #createDependenceInformation('./data/origin/Restaurants_Train_v2.xml','dependences/re_train.dep')
