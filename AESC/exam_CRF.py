...
#在原始数据里检索Aspect Term，汇报预测aspect预测错误的位置
...
def evaluate(detail=False,d_type='re'): 
    ...
    #构造train_x,trian_y,test_x,test_y
    print('加载并处理训练数据集')
    train_corpus=preProcessing.loadXML('data/origin/%s_Train_v2.xml'%d_name)
    train_bio=preProcessing.createBIOClass(train_corpus.corpus,d_type)     #BIO标注形式
    dep_path='dependences/%s_train.dep'%d_type
    train_bio.createDependenceFeature(dep_path)                            #获取依存特征信息(amod,nsubj,dobj)
    train_X,train_Y=train_bio.getFeaturesAndLabels()                       #所有特征信息
    # print(train_X)        # {'word': 'terrible', 'pos': 'JJ', 'lemm': 'terribl', 'w2v_c': 33, 'w2v_c_c': 66, 'amod_l': False, 'nsubj_r': False, 'dobj_r': False}
    #print(train_Y)       #['O', 'O', 'B', 'O', 'O', 'O', 'O', 'O', 'O']
    ...
    #采用CRF模型训练和测试，并报告BIO准则
    train_X,test_X=crfFormat_X(train_X,test_X)    #将训练集和测试集转化为CRF输入的格式
    train_CRF(train_X,train_Y)                    #训练CRF模型--->函数train_CRF
    predict_Y,tagger=tag_CRF(test_X)              #采用CRF模型进行标注，即CRF模型的预测
    ...
    all_terms=[]  #预测的所有测试文本对应的aspect
    for i in range(len(origin_text_test)):
        # 通过bio_tag来获取Aspect Term      BIO-->aspect word
        all_terms.append(getTermsFromYSeq(predict_Y[i],origin_text_test[i]))
    all_offsets=[]  #预测aspect在文本中的位置
    for i in range(len(origin_text_test)):
        #在原始数据里检索Aspect Term, 汇报预测匹配失败的句子
